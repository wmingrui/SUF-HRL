...
from sufh_rl.datasets import VaihingenMulticlassDataset
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Evaluate released SUF-HRL checkpoint on Vaihingen."
    )
    p.add_argument("--config", default="configs/vaihingen.yaml")
...
def main():
    ...
    raw = compute_miou_macc_oa(hist)

    iou = np.asarray(raw["IoU"], dtype=float)
    acc = np.asarray(raw["Acc"], dtype=float)

    metrics = {
        "protocol":
            "Vaihingen 5-class validation; "
            "classes 0-4 included, clutter/background excluded",
        "mIoU": float(iou[:5].mean()),
        "mAcc": float(acc[:5].mean()),
        "OA": float(raw["OA"]),
        "IoU": iou[:5].tolist(),
        "Acc": acc[:5].tolist(),
    }
